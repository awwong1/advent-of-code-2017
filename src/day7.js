...
function solve (input) {
  ...
  lines.forEach(line => {
    const segments = line.split('->').map(val => val.trim())
    const leftSegment = segments[0]
    const program = leftSegment.split(' ')[0].trim()
    const weight = leftSegment.split(' ')[1].trim()
    weights[program] = +weight.replace('(', '').replace(')', '')
    allPrograms.add(program.trim())
    let children = []
    if (segments.length > 1) {
      children = segments[1].split(', ')
      children.forEach(child => {
        allChildren.add(child.trim())
      })
    }
    hash[program] = children
  })

  const sumWeights = {}
  for (let program in hash) {
    let weight = weights[program]
    let children = [...hash[program]]
    while (children.length) {
      let child = children.pop()
      weight += weights[child]
      children = [...children, ...hash[child]]
    }
    sumWeights[program] = weight
  }
  const diffLister = {}
  for (let program in hash) {
    diffLister[program] = hash[program].map(val => sumWeights[val])
    const removeDups = new Set(diffLister[program])
    if ([...removeDups].length <= 1) {
      continue
    }
    hash[program].forEach(child => {
      console.log(child, weights[child], diffLister[child])
    })
    console.log(diffLister[program])
    console.log(removeDups)
  }

  // console.log(diffLister)
...
console.log(solve(input)) // tknk

const tInput = `mmqyju (156) -> rjzvwv, noybkx
...
console.log(solve(tInput))
module.exports = { solve }
console.log(1161-9)


// barf I forgot how to code today
